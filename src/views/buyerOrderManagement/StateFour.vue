        ...
        <li class="container_1" v-for="(order,index) in allOrders" @click="toGoodDetail(order.goodId)" :key="index">
          ...
          <div class="container1-2" style="overflow:hidden;">
            ...
            <div style="overflow-y: scroll;overflow-x: hidden;white-space: pre-line;">
              <small v-html="order.goodPrice"></small>
            </div>
          </div>
          <span class="container1-3">
            <h1 style="color: black;font-size: 40px;margin-top: 7%;"><b>{{order.number}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;￥{{order.goodPrice}}</b></h1>
<!--            <button class="button1" @click.stop @click="afterSalesService(order.orderId)"><span style="line-height: 40px;">申请售后</span></button>-->
          </span>
        </li>
      </ul>
    </div>
...
<script>
  ...
  import {buyerShowOrders,afterSalesService} from "../../api";
