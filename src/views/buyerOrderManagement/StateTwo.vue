...
<!--        <el-col :span="4"><div class="grid-content bg-purple" style="font-size: 20px;line-height: 36px;" @click="toStateOne">待支付</div></el-col>-->
    ...
    <div class="container1">
      ...
      <div v-if="allOrders.length===0">
        ...
        <div style="text-align: center;">
          <span style="text-align: center">暂无待发货的订单哦 /(ㄒoㄒ)/~~</span>
        </div>
      </div>
      <ul    :loading="loadings.table" >
        <li class="container_1" v-for="(order,index) in allOrders" @click="toGoodDetail(order.goodId)" :key="index">
          ...
          <span class="container1-3">
            ...
            <button class="button2" @click.stop @click="orderStatus(order.stmt)"><span style="line-height: 40px;">订单状态</span></button>
            <button class="button1" @click.stop @click="buyerCancelAnOrder(order.orderId)"><span style="line-height: 40px;">取消订单</span></button>
          </span>
        </li>
        <el-dialog
          title="订单状态"
          ...
          width="30%">
          <el-steps :space="200" :active="stmt" finish-status="success">
  ...
  export default {
    ...
    data() {
      return {
        ...
        //     "2020年 [16]  10月，BTS在获奖感言中提及朝鲜战争，表示“会永远铭记两国（韩美）共同经历的苦难历史”。因而引发中国网友和粉丝不满； [17-18]  10月12日，外交部发言人赵立坚回应此事时表示：以史为鉴、面向未来、珍爱和平、促进友好，应该是我们共同的追求，值得我们共同努力。",goodPrice:77.77,number:2,stmt:4},],
        dialogVisible: false,
        stmt:0,
      }
    },
