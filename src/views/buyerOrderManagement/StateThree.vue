...
<!--        <el-col :span="4"><div class="grid-content bg-purple" style="font-size: 20px;line-height: 36px;" @click="toStateOne">待支付</div></el-col>-->
        ...
        <el-col :span="4"><div class="grid-content bg-purple-dark" style="font-size: 20px;line-height: 36px;">已发货</div></el-col>
    ...
    <div class="container1">
      ...
      <div v-if="allOrders.length===0">
        ...
        <div style="text-align: center;">
          <span style="text-align: center">暂无已发货的订单哦 /(ㄒoㄒ)/~~</span>
        </div>
      </div>
      <ul    :loading="loadings.table" >
        <li class="container_1" v-for="(order,index) in allOrders" @click="toGoodDetail(order.goodId)" :key="index">
          ...
          <span class="container1-3">
            ...
            <el-button class="button2" type="text" @click.stop @click="buyerConfirmReceipt(order.orderId)"><span style="color: black;">确认收货</span></el-button>
            <button class="button1" @click.stop @click="trackingInformation(order.trackingNumber)"><span style="line-height: 40px;">查看物流单号</span></button>
          </span>
        </li>
      </ul>
    </div>
...
<script>
  ...
  import {buyerConfirmReceipt, buyerShowOrders, buyerCancelAnOrder,} from "../../api";
  ...
  export default {
    ...
    methods:{
      ...
      trackingInformation(trackingNumber){
        this.$alert(trackingNumber,'物流单号', {
          confirmButtonText: '确定',
        });
      },
    }
